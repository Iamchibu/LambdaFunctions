...
class FunctionalExperiments {
    ...
    public func higherOrderFunction(operation: (Int64) -> Int64) -> Int64 {
        return (1...self.range).reduce(0, { $0 + operation(Int64($1)) })
    }

    // Function types
    public lazy var functionTypes: (Int64) -> Int64 = { value in
        return (1...self.range).reduce(0, { $0 + (Int64($1) * value) })
    }
    ...
    lazy var lambdaExpressionSyntax: () -> Int64 = { (1...self.range).reduce(0, +) }
    ...
    public lazy var itImplicitParameter: (Int64) -> Int64 = { it in
        return (1...it).reduce(0, +)
    }

    // Returning a value from a lambda expression
    public lazy var lambdaWithReturn: (Int64) -> Int64 = {_ in
        return Int64((1...self.range).reduce(0, { $0 + ($1 * 1) }))
    }

    // Underscore for unused variables
    public lazy var unusedVariable: (Int64) -> Int64 = { _ in
        return Int64((1...self.range).reduce(0, +))
    }

    // Function literals with receiver
    public lazy var functionLiteralWithReceiver: (Int64) -> Int64 = { receiver in
    ...
    public func functionTypesWithoutLambda(value: Int64) -> Int64 {
        return (1...self.range).reduce(0) { accumulatedResult, currentElement in
