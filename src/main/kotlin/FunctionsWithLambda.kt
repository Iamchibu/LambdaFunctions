class FunctionsWithLambda {
    // Global constant
    val RANGE = 2000

    // Higher-order functions
    private fun higherOrderFunction(operation: (Int) -> Int):
...
//    println("Sum within the function: $result")
    ...
    // Function types
    val functionTypes: (Int) -> Int =

        { value ->
            val result = (1..RANGE).sumBy {
                it * value
            }
//    println("Sum within the function types: $result")
    ...
    val instantiatingFunctionType: (Int) -> Int =

        { value ->
            val result = higherOrderFunction {
                it * value
            }
//    println("Sum within the instantiating function type: $result")
    ...
    val invokingFunctionTypeInstance: (Int) -> Int =
...
//    println("Sum within the invoking function type instance: $result")
    ...
    // Inline functions
    private inline fun inlineFunctions(operation: (Int) -> Int):
...
//    println("Sum within the inline function: $result")
    ...
    val lambdaExpressionSyntax: (Int) -> Int =
    ...
    private fun higherOrderFunctionWithTrailingLambda(operation: (Int) -> Int, trailing: () -> Unit):
...
//    println("Sum within the function with trailing lambda: $result")
    ...
    val itImplicitParameter: (Int) -> Int =
...
//    println("Sum within the function literal with receiver: $result")
    ...
    fun functionTypesWithoutLambda(value: Int):
    ...
    fun instantiatingFunctionTypeWithoutLambda(value: Int):
    ...
    fun invokingFunctionTypeInstanceWithoutLambda():
    ...
    fun higherOrderFunctionWithTrailingLambdaWithoutLambda():
    ...
    fun itImplicitParameterWithoutLambda(value: Int):
    ...
    fun lambdaWithReturnWithoutLambda():
    ...
    fun unusedVariableWithoutLambda():
    ...
    fun functionLiteralWithReceiverWithoutLambda(receiver: Int):
    ...
    fun main() {
        val functionsList: List<Pair<String, () -> Int>> = listOf(
            ...
            "Instantiating function type" to {
                instantiatingFunctionType(1)
            },
            "Invoking function type instance" to {
            ...
            "Lambda expression syntax" to {
                lambdaExpressionSyntax(1)
            },
            "Passing trailing lambda" to {
                higherOrderFunctionWithTrailingLambda({ it * 1 }) {
                }
            },
            "It: implicit name of a single parameter" to {
                itImplicitParameter(1)
            },
            "Returning value from a lambda" to {
                lambdaWithReturn(1)
            },
            "Underscore for unused variables" to {
                unusedVariable(1)
            },
            "Function literals with receiver" to {
                1.functionLiteralWithReceiver()
            }
        )

        functionsList.forEach { (functionName, function) ->
            val (time, energy, memory) = measureExecution {
                function.invoke()
            }
